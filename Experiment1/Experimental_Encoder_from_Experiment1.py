...
from Experimental_neurons import InfomorphicDense
...
class Feed_Forward(tf.keras.layers.Layer):
    def __init__(self, d_model, ff_dim):
        ...
        self.ffn = tf.keras.Sequential([
            InfomorphicDense(ff_dim, activation="relu", trivariate=False),
            Dense(d_model)
        ])
...
history = model.fit(X_train, y_train, epochs=EPOCHS, validation_data=(X_test, y_test), verbose=1, callbacks = [es, f1_callback], shuffle=True)
